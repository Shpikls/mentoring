...
class Loader {
    ...
    errorHandler(res: Response) {
        if (!res.ok) {
            if (res.status === ResponseStatusCode.Unauthorized || res.status === ResponseStatusCode.NotFound)
    ...
    load(method: 'GET', endpoint: string, callback: (data?: NewsData) => void, options = {}) {
